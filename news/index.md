...
- Improve documentation of `roi` parameter in `sits_mosaic` and
  `sits_plot`
...
- Review
  [`sits_get_data()`](https://e-sensing.github.io/sits/reference/sits_get_data.md)
  implementation
- Improvements in
...
- Fix
  ...
  multicores operations
- Add WebGL as default engine in
...
- Support for multiple tiling system in
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md),
  including MGRS and Brazil Data Cube grids
- Review
  [`sits_merge()`](https://e-sensing.github.io/sits/reference/sits_merge.md)
  implementation to better handle multiple scenario cases
- Support `roi` when plotting data cubes
- Refactor
  [`sits_cube_copy()`](https://e-sensing.github.io/sits/reference/sits_cube_copy.md)
  to improve timeout handling and efficiency
...
- Add support to `SpatExtent` object from terra as `roi` in
...
- Enhance
  [`sits_classify()`](https://e-sensing.github.io/sits/reference/sits_classify.md)
  performance with segments classification
...
- Fix
  [`sits_merge()`](https://e-sensing.github.io/sits/reference/sits_merge.md)
  function that was not merging `SAR` and `OPTICAL` cubes
- Rename n_input_pixels back to input_pixels for compatibility with
  models trained in old versions of the package
- Fix torch usage in Apple M3
- Fix date parameter usage in
  [`sits_view()`](https://e-sensing.github.io/sits/reference/sits_view.md)
- Improve [`plot()`](https://e-sensing.github.io/sits/reference/plot.md)
...
- Include support for DEA products SENTINEL-1-RTC, LS5-SR, LS7-SR,
  LS9-SR, ALOS-PALSAR-MOSAIC, NDVI ANOMALY, DAILY CHIRPS, MONTHLY CHIRPS
  and DEM-30
...
- Include parameter tile to select data from Sentinel-1 and Sentinel-2
  (DEAFRICA)
...
- Improve performance of
  ...
  using chunk parallelization
- Include uncertainty measures for vector probability cubes
- New
  [`sits_clean()`](https://e-sensing.github.io/sits/reference/sits_clean.md)
  function to improve classified maps
- New functions
  [`sits_sampling_design()`](https://e-sensing.github.io/sits/reference/sits_sampling_design.md)
  and
  [`sits_stratified_sampling()`](https://e-sensing.github.io/sits/reference/sits_stratified_sampling.md)
- New
  [`sits_reduce()`](https://e-sensing.github.io/sits/reference/sits_reduce.md)
  function
...
- New version of
  [`sits_get_data()`](https://e-sensing.github.io/sits/reference/sits_get_data.md)
  to extract average values of time series based on segments
- Support for Harmonized Landsat Sentinel (HLS) collections from NASA
- Support for probability cubes and uncertainty cubes in
  [`sits_view()`](https://e-sensing.github.io/sits/reference/sits_view.md)
- New [`summary()`](https://rdrr.io/r/base/summary.html) function to
  show details of data cubes and time series tibbles
- General big fixes
...
- New
  [`sits_mosaic()`](https://e-sensing.github.io/sits/reference/sits_mosaic.md)
  function for improving visualization of large data sets
- Add support to cubes with no cloud coverage information in
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
- Improve
  [`sits_cube_copy()`](https://e-sensing.github.io/sits/reference/sits_cube_copy.md)
...
- Improve
  [`sits_combine_predictions()`](https://e-sensing.github.io/sits/reference/sits_combine_predictions.md)
...
- Fix `.raster_file_blocksize.terra()` bug (issue
  [\#918](https://github.com/e-sensing/sits/issues/918))

#### Hotfix version 1.2.0-3

- Fix `stars` proxy bug (issue
  [\#902](https://github.com/e-sensing/sits/issues/902))
- Fix `purrr` cross deprecation
- Fix `ggplot2` aes_string deprecation

#### Hotfix version 1.2.0-2

- Fix `tibble` subsetting bug (issue
  [\#893](https://github.com/e-sensing/sits/issues/893))

#### Hotfix version 1.2.0-1

- Fix
  [`sits_som_clean_samples()`](https://e-sensing.github.io/sits/reference/sits_som_clean_samples.md)
  bug (issue [\#890](https://github.com/e-sensing/sits/issues/890))

#### New features in SITS version 1.2.0

- [`sits_get_data()`](https://e-sensing.github.io/sits/reference/sits_get_data.md)
  can be used to retrieve samples in classified cube
- Support for mixture models
  ([`sits_mixture_model()`](https://e-sensing.github.io/sits/reference/sits_mixture_model.md))
...
- Fix support to BDC cubes in
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
  (issue [\#848](https://github.com/e-sensing/sits/issues/848))
- Fix support to classified_image cubes in `sits_labels()<-` (issue
  [\#846](https://github.com/e-sensing/sits/issues/846))
...
- Fix out of memory error in
  ...
  and
  [`sits_smooth()`](https://e-sensing.github.io/sits/reference/sits_smooth.md)
  (issue [\#850](https://github.com/e-sensing/sits/issues/850))
...
- Fix resume feature in
  [`sits_classify()`](https://e-sensing.github.io/sits/reference/sits_classify.md)
  on BDC cubes (issue
  [\#844](https://github.com/e-sensing/sits/issues/844))
...
- Improve sits_values() function (issue
  [\#810](https://github.com/e-sensing/sits/issues/810))
- Fix sits_reduce_imbalance() function (issue
  [\#809](https://github.com/e-sensing/sits/issues/809))

#### Hotfix version 1.1.0-1

- Fix sits_accuracy() function (issue
  [\#807](https://github.com/e-sensing/sits/issues/807))

#### New features in SITS version 1.1.0

- Introduced support to kernel functions in `sits_apply`
- Introduced new function `sits_mixture_model` for spectral mixture
  analysis
- Support for the Swiss Data Cube
  ([swissdatacube.org](https://www.swissdatacube.org/))
...
- Introduced
  [`sits_uncertainty_sampling()`](https://e-sensing.github.io/sits/reference/sits_uncertainty_sampling.md)
  for active learning
- Introduced `sits_confidence_samples()` for semi-supervised learning
- Introduced
  [`sits_geo_dist()`](https://e-sensing.github.io/sits/reference/sits_geo_dist.md)
  to generate samples-samples and samples-predicted plot
- Introduced
  [`sits_tuning()`](https://e-sensing.github.io/sits/reference/sits_tuning.md)
  for random search of machine learning parameters
- Introduced
  [`sits_reduce_imbalance()`](https://e-sensing.github.io/sits/reference/sits_reduce_imbalance.md)
  function to balance class samples
- Introduced
  [`sits_as_sf()`](https://e-sensing.github.io/sits/reference/sits_as_sf.md)
  to convert a sits tibble to a sf object
...
- New types of
  ...
  `least` confidence and `margin` of confidence

#### Improvements in SITS version 0.17.0

- Implement parallel processing for
  [`sits_kfold_validate()`](https://e-sensing.github.io/sits/reference/sits_kfold_validate.md)
- Change `data` to `samples` in sits machine learning classifiers (NOTE:
  models trained in previous versions is no longer supported)
- Change deep learning functions to snake case
- Remove `file` parameter in
  [`sits_get_data()`](https://e-sensing.github.io/sits/reference/sits_get_data.md)
  function
...
- reimplemented all deep learning functions using `torch` package and
  remove `keras` dependence
...
- Improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
  to reach production level quality
- Improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
  to use C++ internal functions
- Include improved version of gdalcubes
- Improve
  [`sits_cube()`](https://e-sensing.github.io/sits/reference/sits_cube.md)
  to open results cube
- Update [`plot()`](https://e-sensing.github.io/sits/reference/plot.md)
  parameters on raster cubes
...
- Improve
  [`sits_get_data()`](https://e-sensing.github.io/sits/reference/sits_get_data.md)
  to accept tibbles
- Remove multiples progress bar from
  [`sits_cube()`](https://e-sensing.github.io/sits/reference/sits_cube.md)
- Improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
  to process in parallel by tiles, bands, and dates
- Improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
...
- Now `sits_regularize` has a fault tolerance system, so that if there
  is a processing error the function will delete the malformed files and
  create them again.
...
- Include
  [`sits_apply()`](https://e-sensing.github.io/sits/reference/sits_apply.md)
  function to generate new bands from existing ones;
- Improve
  [`sits_accuracy()`](https://e-sensing.github.io/sits/reference/sits_accuracy.md)
  function to work with multiple cubes;
- Add band parameter to
  [`sits_view()`](https://e-sensing.github.io/sits/reference/sits_view.md)
- Introduce
  [`sits_uncertainty()`](https://e-sensing.github.io/sits/reference/sits_uncertainty.md)
  function to provide uncertainty measure to probability maps;
- Improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md)
...
- Update README.Rmd
- Support
  [`sits_timeline()`](https://e-sensing.github.io/sits/reference/sits_timeline.md)
  to sits model objects
- Update drone image
...
- Change `gdalcubes_chunk_size` in “config.yml” to improve
  [`sits_regularize()`](https://e-sensing.github.io/sits/reference/sits_regularize.md).

#### New features in SITS version 0.15.0-3

- Fix bug in `.source_collection_access_test` to pass ellipsis to
...
- Fix bug in `sits_timeline` for cubes that do not have the same
  temporal extent.

#### New features in SITS version 0.15.0

- Support for regularization of collections in DEAFRICA and USGS
  improvement
...
- Update
  [`sits_list_collections()`](https://e-sensing.github.io/sits/reference/sits_list_collections.md)
  to indicate open data collection
...
- fix
  [`sits_bbox()`](https://e-sensing.github.io/sits/reference/sits_bbox.md)
  function
...
- Now the plot of a classified cube requires a legend or a palette if
  the labels are not in the default sits palette.
...
- Remove
  ...
  function parameters `satellite` and `sensor`
- Add parameters `origin` and `collection` to
  [`sits_cube.local_cube()`](https://e-sensing.github.io/sits/reference/sits_cube.local_cube.md)
  function
...
- Improve selection using `roi` parameter in
  [`sits_classify()`](https://e-sensing.github.io/sits/reference/sits_classify.md)
  function
...
- Time series tibbles and data cube metadata can now be saved and read
  in SQLite
...
- Improvement in code coverage: most of the code has more than 90%
  coverage

#### New features in SITS version 0.9.3

- Improvements in reading shapefiles: using sampling to retrieve time
  series inside polygons
- Improvement is plotting: uses overloading to the “plot” function

#### New features in SITS version 0.9.2

- Raster classification results can now have versions: a new parameter
  “version” has been included in the `sits_classify` function.
...
- New deep learning models for time series: 1D convolutional neural
  networks (`sits_FCN`), combining 1D CNN and multi-layer perceptron
  networks (`sits_TempCNN`), 1D version of ResNet (`sits_ResNet`), and
  combination of long-short term memory (LSTM) and 1D CNN
  (`sits_LSTM_FCN`).
...
- From version 0.8 onwards, the package has been designed to work with
  data cubes. All references to “coverage” have been replaced by
  references to “cubes”.

- The classification of raster images using `sits_classify` now produces
  images with the information on the probability of each class for each
  pixel. This allows more flexibility in the options for labeling the
  resulting probability raster files.

- The function `sits_label_classification` has been introduced to
  generate a labelled image from the class probability files, with
  optional smoothing. The choices are `smoothing = none` (default),
...
- To better define a cube, the metadata tibble associated to a cube
  ...
  provided, these parameters are inferred for the `sits` configuration
  file.

- The functions that do data transformations, such as
  `sits_tasseled_cap` and `sits_savi` now require a `sensor` parameter
  (“MODIS” is the default)

- Functions `sits_bands` and `sits_labels` now work for both tibbles
  with time series and data cubes.

#### Configuration file

- The SITS configuration file has been improved to include information
  about web service providers, satellites and sensor parameters. Please
  use `sits_show_config()` to see the default contents. Users can
  override these parameters or add their own by creating a `config.yml`
  file in their home directory.

#### Examples and Demos

- Examples and demos that include classification of raster files now use
  the `inSitu` R package, available using
...
- `sits_raster_classification` has been removed. Please use
  `sits_classify`.

- In `sits_classify`, the parameter `out_prefix` has been changed to
  `output_dir`, to allow better control of the directory on which to
  write.

- `sits_bayes_smooth` has been removed. Please use
  `sits_label_classification` with `smoothing = bayesian`.

- To define a cube based on local files, `service = RASTER` has been
  replaced by `service = LOCALHOST`.

#### Improvements and fixes

- For programmers only: The `sits_cube.R` file now includes many
  convenience functions to avoid using cumbersome indexes to files and
  vector: `.sits_raster_params`, `.sits_cube_all_robjs`,
  `.sits_class_band_name`, `.sits_cube_bands`, `.sits_cube_service`,
  `.sits_cube_file`, `.sits_cube_files`, `.sits_cube_labels`,
  `.sits_cube_timeline`, `.sits_cube_robj`, `.sits_cube_all_robjs`,
  `.sits_cube_missing_values`, `.sits_cube_minimum_values`,
  `.sits_cube_maximum_values`, `.sits_cube_scale_factors`,
  `.sits_files_robj`. Please look at the documentation provided in the
  `sits_cube.R` file.

- For programmers only: The metadata that describes the data cube no
  longer stores the raster objects associated to the files associated
  with the cube.
