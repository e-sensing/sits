...
test_that("Multi-year, single core classification", {
    ...
    sinop_probs <- sits_classify(sinop, rfor_model,
                                 output_dir = tempdir(),
...
test_that("Multi-year, multi-core classification", {
    ...
    sinop_probs <- sits_classify(sinop, svm_model,
                                 output_dir = tempdir(),
    ...
    expect_true(all(file.remove(unlist(sinop_majority_bayes$file_info[[1]]$path))))
})


test_that("One-year, single core classification", {
    samples_mt_2bands <- sits_select(samples_mt_4bands, bands = c("NDVI", "EVI"))
    dl_model <- sits_train(samples_mt_2bands, sits_deeplearning(
...
test_that("One-year, multicore classification", {
    ...
    sinop_2014_probs <- sits_classify(sinop_2014, svm_model,
                                      output_dir = tempdir(),
                                      memsize = 4, multicores = 2)
