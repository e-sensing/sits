...
test_that("User functions", {
  ...
  expect_equal(
    sits:::.source_collection_tile_check("MSPC", "LANDSAT-8-C2-L2", "232067"),
    NULL
  )
...
test_that("Configs WTSS", {
  ...
  expect_error(
    .config_get(key = c(
      "sources", "BDC", "collections", "CB4_64-1",
      "access_vars"
    ))
  )
  ...
  expect_error(
    ...
    paste0(
      "key 'sources\\$BDC\\$collections\\$CB4_64-1\\$bands",
      "\\$NDVI\\$zzz' not found"
    )
  )

  expect_false(
    .source_cloud_bit_mask(
      source = "BDC",
