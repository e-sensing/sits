test_that("Creating a dendrogram and clustering the results", {
    ...
    dendro <- sits:::.sits_cluster_dendrogram(cerrado_2classes,
        bands = c("NDVI", "EVI")
    )
    expect_true(dendro@distmat[1, 2] > 3.0)

    vec <- sits:::.sits_cluster_dendro_bestcut(cerrado_2classes, dendro)
    ...
    result <- sits:::.sits_cluster_validity(clusters)
