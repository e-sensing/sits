test_that("One-year, multicore classification with ROI", {
    ...
    rc_obj <- .raster_api_open_rast(sinop_probs$file_info[[1]]$path[[1]])
    expect_true(.raster_api_nrows(rc_obj) == sinop_probs$nrows)
    ...
    max_lyr2 <- max(.raster_api_get_values(rc_obj)[, 2])
    expect_true(max_lyr2 <= 10000)

    max_lyr3 <- max(.raster_api_get_values(rc_obj)[, 3])
