...
In our example above, we can separate our samples by cutting the dendrogram tree at some level to achieve a balancing between two objectives: get clusters as large as possible, and get clusters as homogeneous as possible. To help this process, `SITS` provides `sits_dendro_validity()` which computes a series of external validity indexes for a given range of number  [], there is no an automatic way of achieving it [@Liao2005][@Aghabozorgi2015]. This process can be realised visually by looking the dendrogram or assisted by an *external cluster validity indexes* such as Rand Index ... as shows the cut height dashed line in our example.
