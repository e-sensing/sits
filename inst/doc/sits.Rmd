...
In our example above, we can separate our samples by cutting the dendrogram tree at some level to achieve a balancing of two objectives: get as large clusters as possible, and get as homogeneous clusters as possible. Despite the existence of various indices that try to capture this balancing relation [], there is no an automatic way of achieving it [@Liao2005][@Aghabozorgi2015]. This process can be realised visually by looking the dendrogram or assisted by an *external cluster validity indexes* such as Rand Index ... as shows the cut height dashed line in our example.
