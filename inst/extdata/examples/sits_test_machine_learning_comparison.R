...
conf_svm.tb <- sits_kfold_validate(embrapa.tb, folds = 5, multicores = 8, 
                                   pt_method   = sits_gam(),
...
sits_save (conf_svm.tb, file = "/home/gilberto/sits/inst/extdata/results/svm_5folds.json")

# test accuracy of TWDTW to measure distances
conf_lda.tb <- sits_kfold_validate(embrapa.tb, folds = 5, multicores = 8,
                                     pt_method   = sits_gam(),
                                     dist_method = sits_TWDTW_distances(multicores = 8),
                                     tr_method   = sits_lda ())
...
sits_save (conf_lda.tb, file = "/home/gilberto/sits/inst/extdata/results/lda_5folds.json")

# Linear Discriminant Analysis
conf_qda.tb <- sits_kfold_validate(embrapa.tb, folds = 5, multicores = 8,
                                     pt_method   = sits_gam(),
                                     dist_method = sits_TWDTW_distances(multicores = 8),
                                     tr_method   = sits_qda())
...
sits_save (conf_qda.tb, file = "/home/gilberto/sits/inst/extdata/results/qda_5folds.json")

# "multinomial log-linear (mlr)
conf_mlr.tb <- sits_kfold_validate(embrapa.tb, folds = 5, multicores = 8,
                                    pt_method   = sits_gam(),
                                    dist_method = sits_TWDTW_distances(multicores = 8),
                                    tr_method   = sits_mlr())
...
sits_save (conf_mlr.tb, file = "/home/gilberto/sits/inst/extdata/results/mlr_5folds.json")

# generalized liner model (glm)
conf_glm.tb <- sits_kfold_validate(embrapa.tb, folds = 5, multicores = 8,
                                   pt_method   = sits_gam(),
                                   dist_method = sits_TWDTW_distances(multicores = 8),
                                   tr_method   = sits_glm())
...
sits_save (conf_glm.tb, file = "/home/gilberto/sits/inst/extdata/results/glm_5folds.json")
