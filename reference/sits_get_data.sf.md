...
their metadata. The `samples` parameter must be a `sf` object in POINT
or POLYGON geometry. If `start_date` and `end_date` are not informed,
the function uses these data from the cube.
