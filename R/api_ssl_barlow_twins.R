# ---- Barlow Twins self-supervised learning helpers ----

#' @title Split samples into train/val for self-supervised Barlow Twins
#' @name .ssl_barlow_twins_data_split
#' @keywords internal
#' @noRd
...
#' indices. No pre-computed pairs are built because the self-supervised
#' Barlow Twins method creates views on-the-fly via resampling augmentation.
#'
...
#'   element is itself a list with component \code{feats} (a numeric matrix
#'   of shape \code{[n_split, n_times * n_bands]} — normalized features).
#'
.ssl_barlow_twins_data_split <- function(samples, validation_split) {
...
#' @title Torch Dataset for Barlow Twins with resampling augmentation
#' @name .ssl_barlow_twins_resampling_dataset
#' @keywords internal
#' @noRd
#' @author Gilberto Camara, \email{gilberto.camara@@inpe.br}
#'
...
#' sample, two augmented views are created on-the-fly using the resampling
#' strategy of Saget et al. (2025) via \code{.ssl_apply_resampling()}.
#'
...
#' }
#'
#' @param split   List with element \code{feats}, a numeric matrix of shape
#'   \code{[n_samples, n_times * n_bands]}.
...
#'
.ssl_barlow_twins_resampling_dataset <- torch::dataset(
    name = ".BarlowTwinsResamplingDataset",
