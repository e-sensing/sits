...
#' @title Measure information content on model based clusterinbg
#' @name sits_mclust_info
#' @author Gilberto Camara, \email{gilberto.camara@@inpe.br}
#'
#' @description This function is part the support in `sits` for model-based clustering.
#' The general idea of model-based clustering is that the input data can be modelled as
#' a finite mixture model of Gaussian distributions, with different covariance structures
#' and different members of misture components. The package `sits` provides a convenient front-end
#' to the `mclust` package.
#'
#' This function measures the bayesian information criterion (BIC) for a set
#' of parametrized Gaussian mixture models fitted by an expectation-maximization (EM) method.
#' It plots the different models for the chosen number of clusters and informs the best models
#' based on the BIC.
#'
#' @param data.tb           a SITS tibble with labelled samples
#' @param nclusters         the maximum number of clusters assumed to exist in the data
#' @return BIC              a `mclust` structure with the bayesian information criterion.
...
#' # load a simple data set with two classes
#' data(cerrado_2classes)
#' # calculate the BIC for different model shapes
#' BIC <- sits_mclust_info(cerrado_2classes, nclusters = 4)
#' }
#' @export
sits_mclust_info <- function(data.tb, nclusters = 9) {
    # set the distance function
    dist.tb <- sits_distances(data.tb)

    # create the model vector
    X <- dist.tb[,-(1:2)]

    # obtain the bayesian information criteria for different model shapes
    BIC <- mclust::mclustBIC(X, G = 1:nclusters)
    # plot the results
    graphics::plot(BIC)
...
#' @title Compute the clusters for a chosen model
#' @name sits_mclust_compute
#' @author Gilberto Camara, \email{gilberto.camara@@inpe.br}
#'
#' @description This function is part the support in `sits` for model-based clustering.
#' The general idea of model-based clustering is that the input data can be modelled as
...
#' to the `mclust` package.
#'
#' This function computes the cluster values assigned to each input samples, based on
#' a model chosen by the user. Users should first run \code{\link[sits]{sits_mclust_info}}
#' the get the bayesian information criteria (BIC) for each different model structure.
#' Then, they should select one of models suggested by this function to assign a
#' cluster to each input of the SITS tibble. The function will also assign an uncertainity value
#' to the predicted cluster.
#'
#' Usually, the model_name will be one of {"EEI", "VII", "EEI", "VEI", "EVI", "VVI",
#' "EEE", "EVE", "VEE", "VVE", "EEV", "VEV", "EVV", "VVV"}. See the reference for more details.
#'
#' @param data.tb           a SITS tibble with labelled samples.
#' @param nclusters          the maximum number of clusters assumed to exist in the data.
#' @param model_name        one of the `mclust` model structures.
#' @return data.tb          a SITS tibble with two additional columns: "cluster" (integer) and
#'                          "uncertainity" (double).
#'
#' @references   Scrucca L, Fop M, Murphy TB, Raftery AE.
#'               mclust 5: Clustering, Classification and Density Estimation Using Gaussian
...
#' # calculate the BIC for different model shapes
#' BIC <- sits_mclust_info(cerrado_2classes, nclusters = 4)
#' # select one model for processing
#' cluster.tb <- sits_mclust_compute(cerrado_2classes, nclusters = 4, model_name = "EEE")
#' # show clusters samples frequency
#' sits_cluster_frequency(clusters.tb)
#' }
#' @export
sits_mclust_compute <- function(data.tb, nclusters = 9, model_name = NULL){

    # compute the distance vector from the data
    dist.tb <- sits_distances(data.tb)

    # create the model vector
    X <- dist.tb[,-(1:2)]

    # compute the clusters using the mclust function
    myMclust <- mclust::Mclust(X, modelNames = model_name, G = 1:nclusters)

    # insert the predicted cluster number and the associated uncertainity in the data tibble
    data.tb$cluster = as.integer(myMclust$classification)
    data.tb$uncertainity <- as.double(myMclust$uncertainty)
