...
sits_label_classification <- function(cube,
    ...
    assertthat::assert_that(smoothing %in% c(
        "none", "bayesian",
        "majority", "bayesian+majority"
    ),
    msg = "sits_label_classification: unsupported smoothing method"
    )
    ...
    purrr::map2(in_files, out_files,
                function(in_file, out_file) {
                    for (b in 1:n_labels) {
                        ...
                        # for each class, compute the smoothing estimator (if required)
                        if (smoothing == "bayesian" ||
                            smoothing == "bayesian+majority")
                            # get smoothed values
                            values[, b] <- bayes_estimator_class(data,
                                                                 window,
                                                                 variance
                            )
                        else
                    ...
                    cube_labels <- .sits_raster_api_write_lab(
                        cube = cube_labels,
                        values = values,
                        smoothing = smoothing,
                        filename = out_file
