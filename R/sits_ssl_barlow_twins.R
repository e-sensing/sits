#' @title Self-supervised Barlow Twins pre-training with resampling augmentation
#' @name sits_ssl_barlow_twins
#'
#' @description
#' Self-supervised pre-training using the Barlow Twins loss and a torch
...
#' upsampled, two disjoint subsequences are drawn with a temporal coverage
#' constraint, and each is resampled back to the original length.  No labels
#' are required.
#'
#' Both views are passed through a shared encoder and projector.  The Barlow
#' Twins loss makes the cross-correlation matrix of the two views' embeddings
#' close to the identity: the diagonal -> 1 (invariance) and the off-diagonal
#' -> 0 (redundancy reduction).  No negatives are required.
#'
#' Unlike the supervised \code{\link[sits]{sits_barlow_twins}} (which pairs two
#' different samples that share a class label), this function is fully
#' self-supervised: the two views come from the same sample via augmentation,
#' so no labels are used during pre-training.
#'
...
#' \code{function(samples)} that trains a Barlow Twins model and returns a
...
sits_ssl_barlow_twins <- function(samples          = NULL,
    ...
    train_fun <- function(samples) {
        ...
        # ------------------------------------------------------------------
        # Build augmentation-based datasets for Barlow Twins
        #
        ...
        train_ds <- .ssl_barlow_twins_resampling_dataset(
            splits[["train"]],
            n_times = n_times
        )
        val_ds <- .ssl_barlow_twins_resampling_dataset(
            splits[["val"]],
            n_times = n_times
        )
        ...
        barlow_twins_model <- torch::nn_module(
            ...
            initialize = function(encoder, embedding_dim, proj_dim = 128L,
                ...
                # Barlow Twins standardises features inside the loss, so the
                # projector output stays linear.
        ...
        #
        # Two terms:
        #   1. Invariance:            pull the cross-correlation diagonal to 1
        #   2. Redundancy reduction:  push the off-diagonal to 0 (bt_lambda)
        # ------------------------------------------------------------------
