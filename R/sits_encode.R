...
#'    \item{\code{\link[sits]{sits_encode.vector_cube}} is called for
...
#'
#' @export
sits_encode <- function(data, ml_model, ...) {
...
#' @title   encode a segmented data cube
#' @name sits_encode.vector_cube
#' @description
...
#'    used for processing. We recommend using as much memory as possible.
#'
#'    For encodeing vector data cubes created by
...
#' }
#' @export
sits_encode.vector_cube <- function(data,
                                      ml_model, ...,
                                      roi = NULL,
                                      filter_fn = NULL,
                                      impute_fn = impute_linear(),
                                      ...
                                      progress = TRUE) {
    # set caller for error messages
    .check_set_caller("sits_encode_segs")
    # preconditions
    ...
    # Process each tile sequentially
    .cube_foreach_tile(data, function(tile) {
        # encode all the segments for each tile
        .encode_vector_tile(
            tile = tile,
            bands = bands,
            base_bands = base_bands,
...
#' @rdname sits_encode
#' @export
sits_encode.tbl_df <- function(data, ml_model, ...) {
    ...
    sits_encode(data, ml_model, ...)
}
#' @rdname sits_encode
#' @export
sits_encode.derived_cube <- function(data, ml_model, ...) {
    stop(.conf("messages", "sits_encode_derived_cube"))
}
#' @rdname sits_encode
#' @export
sits_encode.default <- function(data, ml_model, ...) {
    data <- tibble::as_tibble(data)
    if (all(.conf("sits_cube_cols") %in% colnames(data))) {
        data <- .cube_find_class(data)
    } else if (all(.conf("sits_tibble_cols") %in% colnames(data))) {
