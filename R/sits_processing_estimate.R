...
#' @name .sits_processing_estimate_classification_time
#' @keywords internal
...
.sits_processing_estimate_classification_time <- function(start_time,
                                                          n_intervals,
                                                          bs,
                                                          block,
                                                          time) {
...
#' @name .sits_processing_estimate_task_time
#' @keywords internal
#' @author Rolf Simoes, \email{rolf.simoes@@inpe.br}
#'
#' @description This function normalizes image values.
...
.sits_processing_estimate_task_time <- function(task, start_time) {
    # compute current time
    current_time <- lubridate::now()
    ...
    elapsed_time <- lubridate::time_length(current_time - start_time,
                                           unit = "minute")
    message(paste0("Elapsed time for task ", task, " : ",
