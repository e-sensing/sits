...
sits_classify_raster <- function(file = NULL,
    ...
    # classify the data
    raster_class.tb <- .sits_classify_multicores2(raster.tb,
                                             raster_class.tb,
                                             time_index.lst,
                                             attr_names,
                                             int_labels,
...
#' @name .sits_classify_multicores2
#' @author Gilberto Camara, \email{gilberto.camara@@inpe.br}
#'
...
#'
.sits_classify_multicores2 <-  function(raster.tb,
                                    raster_class.tb,
                                    time_index.lst,
                                    attr_names,
                                    int_labels,
    ...
    process_block <- function(block_info) {
        ...
        dist.tb <- data.table::data.table("original_row" = rep(1,size),
                                              "reference" = rep("NoClass", size))

        # read the values from the raster bricks
        for (i in 1:length(bricks.vec)) {
            # the readGDAL function returns a matrix
            # the rows of the matrix are the pixels
            # the cols of the matrix are the layers
            values.tb    <- data.table::as.data.table(suppressWarnings(rgdal::readGDAL(bricks.vec[i], offset, region.dim, silent = TRUE))@data)
            ...
            values.tb[is.na(values.tb)] <- minimum_value
            values.tb[values.tb <= minimum_value] <- minimum_value

            # values.mx <- preprocess_data(values.mx, minimum_value, scale_factor)
            # scale the data set
            #values.tb <- values.tb*scale_factor + adj_val
            values.tb <- scale_data(as.matrix(values.tb), scale_factor, adj_val)

            if (smoothing) {
                rows.lst <- lapply(seq_len(nrow(values.tb)), function(i) values.tb[i, ]) %>%
                    lapply(whit)
                values.tb <- do.call(rbind, rows.lst)
            }
            dist.tb <- cbind(dist.tb, values.tb)
        }
