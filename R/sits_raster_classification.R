...
sits_classify_raster <- function(file = NULL,
    ...
    full_data_size <- as.numeric(nrows*ncols*ntimes*nbands*bloat) + as.numeric(pryr::mem_used())
...
.sits_classify_bigdata <-  function(raster.tb,
    ...
    for (t in 1:length(time_index.lst)) {
        ...
        block_size.lst <- .sits_split_block(nrow(dist1.tb), multicores)
