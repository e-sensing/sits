...
#'
.sits_config_cube_generic <- function(type) {
    ...
    classes <- sits_env$config$cube_classes_generic
    names(classes) <- types
...
.sits_config_cube_specific <- function(type) {
    ...
    classes <- sits_env$config$cube_classes_specific
    names(classes) <- types
    return(unname(classes[type]))
}
