...
#' cluster.tb <- sits_cluster_dendro (cerrado_2classes, bands = c("ndvi", "evi"))
#' }
#' @export
sits_cluster_dendro <-  function(data = NULL, bands = NULL, dist_method = "dtw_basic",
    ...
    dendro.obj <- .sits_dendrogram(data, bands, dist_method, linkage, ...)

    # find the best cut for the dendrogram
    if (!silent) message("finding the best cut for the dendrogram...")
    cut.vec <- .sits_dendro_bestcut(data, dendro.obj)
