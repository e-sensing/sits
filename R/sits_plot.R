...
#' }
#' @export
plot.radd_model <- function(x, y) {
    stopifnot(missing(y))
    locs <- dplyr::distinct(x, .data[["longitude"]], .data[["latitude"]])
...
.plot_ggplot_radd <- function(row) {
    ...
    # extract the time series
    data_prob <- dplyr::bind_rows(row$prob_nf)
    # melt the data into long format
    melted_ts <- data_ts |>
        tidyr::pivot_longer(cols = -"Index", names_to = "variable") |>
        as.data.frame()
    data_prob <- data_prob[data_prob$Flag == "Change",]
    change_occur <- NA
    if (nrow(data_prob) > 0) {
        melted_prob_pts <- dplyr::filter(
            melted_ts, .data[["Index"]] %in% data_prob[["Index"]]
        )
        change_occur <- max(data_prob$Index, na.rm = TRUE)
    }
    ...
    if (!is.na(change_occur)) {
        g <- g +
            ggplot2::geom_point(
                data = melted_prob_pts,
                ggplot2::aes(
                    x = .data[["Index"]],
                    y = .data[["value"]],
                    group = .data[["variable"]]
            ...
            ggplot2::geom_vline(
                ggplot2::aes(xintercept = change_occur,
                             linetype = "dotdash"),
                size = 1,
                colour = "#EBB023"

            ) +
            ggplot2::scale_linetype_manual(values = "dotdash", name = "break")
    }
    return(g)
}
