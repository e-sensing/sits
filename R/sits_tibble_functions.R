...
.sits_normalization_param <- function(data.tb, use_IQR = TRUE) {
    ...
    if(use_IQR){
        DT_tend <- DT[, lapply(.SD, median, na.rm = TRUE)]
        DT_disp <- DT[, lapply(.SD, IQR, na.rm = TRUE)]
    }else{
...
#' @author Alber Sanchez, \email{alber.ipia@@inpe.br}
#'
#' @description This function simulates a sits_tibble using random data. The time series are simulated using a cosine function.
#'
#' @param samples       A length-one numeric. The number of samples per label.
...
#' @param lat_mean      A length-one numeric. The standard deviation of the latitude of the random samples. The default is 1.
...
sits_random_tibble <- function(n_samples, label = "label_A", lon_mean = -65,
    ...
    random_st <- tibble::tibble(longitude   = rnorm(n, lon_mean, lon_sd),
                                latitude    = rnorm(n, lat_mean, lat_sd),
...
#' @author Alber Sanchez, \email{alber.ipia@@inpe.br}
#'
#' @description This function takes raonm samples of the polygons given as a
#' sf object.
#'
...
sits_sample_shp <- function(shp.sf,
    ...
    sits:::.sits_test_tibble(shp_samples)
    return(shp_samples)
}
...
.sits_simulate_ts <- function(t_vector, m = 0.5, amplitude = 0.15,
    ...
    noise <- rnorm(length(t_vector), noise_mean, noise_sd)
