...
One of the challenges in land use classification is being able to process large data files. To reduce processing time, users should adjuste the `sits_classify_raster` function according to the capabilities of the server. The package tries to keep memory use to a minimum, and performs garbage collection to free memory as often as possible. Nevertheless, there is an inevitable trade-off between computing time, memory use and I/O operations. The user should specify how much free memory will be available by using the "memsize" parameter. SITS tries to maximize the number of cores available for processing, given the available memory.
