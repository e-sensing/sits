...
$$
depends only on those samples that violates the maximum margin criteria, here captured by the fact that $\alpha_k>0$ only for those points laying either inside the margin band or in the wrong side of the boundary, the so-called *support vectors*. For all other points, $\alpha_k=0$. Hence no one exert any influence on the coefficients determinancy. This property provide SVM a less dependency to outliers. For more details on how to compute $\alpha$ and $\hat{a}_0$, see @Cortes1995, @Hastie2009, and @James2013.
...
$$
one of the most used nonlinear kernels, maps the input space to an infinite dimensional feature space, which would be roughly obtained by expensive calculations through dot-product. Other popular choices for $K$ in the SVM literature are the polynomial kernel ($K(u,v)=(\gamma{u^{\intercal}v}+r)^d$, with parameters $\gamma$, $r$, and $d$), and the linear kernel $K(u,v)=u^{\intercal}v$. The use of kernels are an efficient computational strategy to produce different boundaries shapes in the input attribute space. Moreover, this stragey generally this achieves better training-class separation.
