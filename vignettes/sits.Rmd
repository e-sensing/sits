...
```{r, eval = FALSE}
# a point in the transition forest to pasture and then to agriculture in Mato Grosso
# obtain a time series from the WTSS server for this point
point_ndvi <- sits_get_data(cube      = wtss_cube,
                           longitude = -55.50563, 
                           latitude  = -11.71557,
                           bands     = c("NDVI"))
```
...
```{r, fig.align="center", fig.height=3.1, fig.width=5, fig.cap="NDVI time series fetched from WTSS service."}
...
Doing post-processing using Bayesian smoothing in SITS is straightforward. The result of the `sits_classify` function applied to a data cube is  set of more probability images, one per requested clasification interval. The next step is to apply the `sits_smooth_bayes` function. By default, this function selects the most likely class for each pixel considering a Bayesian estimator that takes into account the neighbours. The following example takes the previously produced classification output and applies a Bayesian smoothing. 

```{r, eval = FALSE}
# smooth the result with a bayesian filter
sinop_bayes <- sits_smooth_bayes(probs, output_dir = tempdir())
