...
The Support Vector Machine (SVM) is a generalization of the *separating hyperplane classifier* [@Hastie2009]. This generalization combines the notions of the *optimal separating hyperplanes*, the *soft margins*, and the *enlargement of the input attribute space* mapping it to non-linear feature space.
...
\end{split}
$$
Once the solution for a such hyperplane is obtained (i.e. values for coeficients $a_0$, $a_1$, $\ldots$, $a_p$), one is able to derive a very simple classification model $f(x)=\hat{a}_0+\hat{a}_1x_{1}+\hat{a}_2x_{2}+\ldots+\hat{a}_px_{p}$ that classifies any test feature vector $x^*$ according to
$$
\begin{split}
y^*=1&\quad f(x^*)>0,\\
y^*=-1&\quad f(x^*)<0.
...
However, the condition for which this classifier has solution is too restrictive, as it needs a linearly separable data set. This is hardly what one may expect in a typical satellite image time series data, where nonlinear relationship between classes and predictors can be find. To overcome this undesired constraint, SVM introduces a *softner term* in the hyperplane optimization problem. This modification allows some observations to be inside the margin band, or even in the incorrect side, violating the maximum margin criteria but increasing the robustness on individual observations influence. 

According to @Cortes1995, the solution for the hyperplane coefficients
$$
\hat{a}_k=\sum_{k=1}^{n}{\omega_{k}x_{k}y_{k}}
$$
depends only on those samples that violates the maximum margin criteria, points that lays either in the margin or in the wrong side of the attribute space, the so-called *support vectors*. All other points does not exert any influence on coefficients determinancy.
