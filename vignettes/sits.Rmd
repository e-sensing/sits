...
coverage_wtss <- 
    sits_coverage(service  = "WTSS-INPE-2", 
                  product  = "MOD13Q1", 
                  name     = "MOD13Q1")
...
The Support Vector Machine (SVM) is a generalization of the *separating hyperplane classifier* [@Hastie2009]. This generalization combines the notions of the *optimal separating hyperplanes*, the *soft margins*, and the *enlargement of the input attribute space* with a nonlinear mapping to a feature space.

In a separating hyperplane formulation, the classifier works only with separable sample sets. It finds a linear boundary in the input attribute space that not only divides two classes but maximizes its margin to them. A boundary is a *hyperplane* that divides the entire attribute space into two parts. Let ${({\rm x}_k, y_k)}$, ${k\in{}(1,\ldots,n)}$ be a set of $n$ observations with attribute vector ${\rm x}_k\in{\mathbb{R}^p}$ and class ${y\in\{-1,1\}}$. Once feature space has $p$ dimensions, the hyperplane has dimension $p-1$ and can be written as
$$
a_0+a_1x_{1}+a_2x_{2}+\ldots+a_px_{p}=0,
$$
where $x_{i}$ is the $i$-th attribute of ${\rm x}$.

If the samples are separable, the hyperplane has a perfect classification property, that is, for all sample ${({\rm x}_k, y_k)}$, the following relation holds
$$
\begin{split}
a_0+a_1x_{k_1}+a_2x_{k_2}+\ldots+a_px_{k_p}<0,&\quad{}{y_k=-1},\\
a_0+a_1x_{k_1}+a_2x_{k_2}+\ldots+a_px_{k_p}>0,&\quad{}{y_k=1}.
\end{split}
$$
Once the solution for a such hyperplane is obtained (*i.e.* values for coeficients $a_0,a_1,\ldots,a_p$), one is able to derive a very simple classification model
$$
f({\rm x})=\hat{a}_0+\hat{{\rm a}}{\rm x}
$$
that classifies any test vector ${\rm x}$ according to
$$
\begin{split}
y=1,&\quad{\rm if }\ f({\rm x})>0,\\
y=-1,&\quad{\rm if }\ f({\rm x})<0.
\end{split}
$$

However, the condition under which this classifier has solution is too restrictive, as it needs a linearly separable training set. This is hardly what one may expect in a typical satellite image time series scenario. To overcome this undesired constraint, SVM introduces a *softner term* in the hyperplane optimization problem. This modification allows some observations to be inside the margins band, or even in the incorrect side of the boundary, violating the maximum margin criteria but increasing its robustness by reducing individual observations influence. Moreover, the solution for the hyperplane coefficients ${\hat{{\rm a}}=(a_1,\ldots,a_n)^\intercal}$, given by
$$
\hat{{\rm a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}{\rm x}_{k}},
$$
depends only on those samples that violates the maximum margin criteria, here captured by the fact that ${\alpha_k>0}$ only for those points laying either inside the margin band or in the wrong side of the boundary, the so-called *support vectors*. For all other points, ${\alpha_k=0}$, and hence none of them exert any influence on the coefficients determination. This property let SVM less sensitive to outliers (for more details on how to compute ${rm \alpha}$ and $\hat{a}_0$, see @Cortes1995, @Hastie2009, and @James2013).

Hyperplanes are linear ${(p-1)}$-dimensional boundaries and define linear partitions in the attribute space. However, sometimes we are faced with nonlinear relationship between attribute vectors and classes. In such cases, linear classification models can suffer to do a good job. An alternative to this limitation is the *enlargement of the attribute space* by a nonlinear mapping $\phi$ to a higher degree feature space. In this manner, the new classification model
$$
f({\rm x})=\hat{a}_0+\hat{{\rm a}}^\intercal\phi({\rm x}),
$$
defines its linear hyperplane on the enlarged feature space which translate to nonlinear boundaries in the original input space. Here, the coefficients are obtained simmilarly by
$$
\hat{{\rm a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\rm x}_{k})},
$$
which substituting in $f$ expression above gives us
$$
f({\rm x})=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\rm x}_{k})^\intercal\phi({\rm x})}.
$$
Moreover, the linearity of the model implies that the term ${\phi({\rm x}_{k})^\intercal\phi({\rm x})}$ can be expressed as a dot product ${\phi({\rm x}_{k})\cdot{}\phi({\rm x})}$ without change its properties.

However, the enlargement of the feature space by $\phi$ can be computationaly expensive. SVM makes use of *kernels* to overcome this limitation. Kernel functions are equivalent to dot products of a larger degree feature space vectors. Hence, we can express $f$ as [@Cortes1995]:
$$
f({\rm x})=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}K({\rm x}_{k}, {\rm x})},
$$
where ${K(u,v)}$ is a kernel function that is equivalent to the dot product ${\phi(u)\cdot\phi(v)}$ (see @Cortes1995). It follows that, we can use kernel functions without the need to compute $\phi$, otherwise it would require massive computations. For example, the radial basis function (RBF) kernel
$$
K({\rm u},{\rm v})=\exp(-\gamma\lVert{{\rm u}-{\rm v}}\rVert^2),
$$
one of the most used nonlinear kernels, maps the input space to an infinite dimensional feature space, which would be roughly obtained by a power series expansion with substantially more calculations. Other popular choices for $K$ in the SVM literature are the polynomial kernel (${K({\rm u},{\rm v})=(\gamma{{\rm u}^{\intercal}{\rm v}}+r)^d}$, with parameters $\gamma$, $r$, and $d$), and the linear kernel ${K({\rm u},{\rm v})={\rm u}^{\intercal}{\rm v}}$. The use of kernels are an efficient computational strategy to produce nonlinear boundaries in the input attribute space generally achieving better training-class separation.
...
Random forests can be used either as a regression or as a classification technique. Here, we are interested in the latter case. Random forests combines two main ideas that emerged in 1990s statistical learning literature: developing population of trees from *stochastic feature selection* [@Ho1995], and *bagging* [@Breiman1996]. It is an *ensemble learning* technique that develops a population of somewhat uncorrelated decision trees that are combined to form a predicting model which is made by a majority vote schema. This aggregating procedure substantialy improves the predictive power of individual trees. Unlike SVM, random forest undertakes a nondeterministic model fitting. 

It starts by growing trees through a bootstrap procedure, that is, a repeatedly random samplings with replacement from which, at every step, a decision tree $T$ is produced considering only some fraction $\lambda \in (0,1)$ of the original observations. The final ensemble is composed by a set of decision trees. 

In random forest algorithm, each growing tree splits its nodes by a *stochastic feature selection*. At each node splitting, it selects $m$ attributes at random from the existing full set of $p$ attributes of the input space. According to a *purity criterion*, the best attribute ${\hat{m} \in \{1,\ldots,p\}}$ is selected among the $m$ candidates (${1 < m \le p}$), which then is used to split that node at some value $S$ of the chosen attribute that minimizes the impurity index. Let ${R_j \subseteq \mathbb{R}^p}$ be the input space region associated with a node $j$. We denote by ${\{{\rm x} \in R_j\}}$ the set of all observations vectors comprehended by the region $R_j$. The splitting step creates a partition on the node region containing two subregions ${R{_\le}_j,R{_>}_j \subset R_j}$ such that
$$
\begin{split}
R{_\le}_j&=\{{\rm x}\ |\ x_{\!\hat{m}} \le S,\ \forall {\rm x} \in R_j \}\\
R{_>}_j&=\{{\rm x}\ |\ x_{\!\hat{m}} > S,\ \forall {\rm x} \in R_j \}.
\end{split}
$$
The splitting process continues until either the number of observations in $|R_j|$ is bellow or equal a given parameter $N_{\!m\!i\!n}$, or the impurity index reaches zero. The determination of the attribute $\hat{m}$ and the value $S$ can be done quickly by scanning the observations. The best pair ${(\hat{m}, S)}$ is that one with the minimum impurity index.

Generally, one can use any impurity criterion such as *Gini*, *cross-entropy*, or *misclassification error* to decide which attribute is to be used to split the node. Let the proportion of observations of class $q$ in node $j$ given by
$$
P_{jq}=\frac{1}{|R_j|}\sum_{{\rm x}_k \in R_j}{\delta_{y_kq}},
$$
where $\delta$ is the Kronecker delta function. The Gini index of a node $j$ can be obtained by
$$
G_{\!j}=\sum_{q \in Q}{P_{jq}(1-P_{jq})},
$$
where $Q$ is set of all class labels. Cross-entropy is the entropy of all classes in a given node $j$ and can be calculated by
$$
E_{\!j}=-\sum_{q \in Q}{P_{jq} \log P_{jq}}.
$$
Finally, the misclassification error impurity index of a node $j$ can be obtained by
$$
M_{\!j}=1-P_{{j\hat q}},
$$
where ${{\hat q}={\rm arg\,max}_{q}{P_{jq}}}$ is the majority class in the node $j$. As a consequence of these formulations, cross-entropy and the Gini index are more sensitive to changes in the node classes proportions than the misclassification error [@Hastie2009].
...
Note that we need to create a coverage `tibble` with some metadata information before do the classification. We have proceeded in the same way with `sits_getdata()` example above, to retrieve WTSS data. The metadata maily concern about which bands and its respective files to connect to. A `timeline` must be provided and it refers to the corresponding date associated with the MOD13Q1 product images used to compose the raster brick. Once created, the coverage can be used either to retrieve time series data from the raster bricks using `sits_getdata()` or to do the raster classification by calling the function `sits_classify_raster()`. The machine learning model and the training data to be used are passed by the arguments `sits_svm()` (default) and `samples_MT_ndvi` parameters. Note that we need to be consistent about the bands informed to the coverage and the bands present in the training samples. The classification result is stored as a set of files begining with `file` prefix.
