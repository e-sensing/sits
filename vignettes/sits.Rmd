...
$$
\begin{split}
f({\bf x})&=\hat{a}_0+\sum_{i=1}^{p}\hat{a}_ix_{i}\\
&=\hat{a}_0+\hat{{\bf a}}{\bf x}
\end{split}
$$
that classifies any test vector ${\bf x}$ according to
$$
\begin{split}
&y=1,&\quad{\rm if }\ f({\bf x})>0,\\
&y=-1,&\quad{\rm if }\ f({\bf x})<0.
\end{split}
$$

However, the condition under which this classifier has solution is too restrictive, as it needs a linearly separable training set. This is hardly what one may expect in a typical satellite image time series scenario. To overcome this undesired constraint, SVM introduces a *softner term* in the hyperplane optimization problem. This modification allows some observations to be inside the margins band, or even in the incorrect side of the boundary, violating the maximum margin criteria but increasing its robustness by reducing individual observations influence. Moreover, the solution for the hyperplane coefficients $\hat{{\bf a}}=(a_1,\ldots,a_n)^\intercal$, given by
$$
\hat{{\bf a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}{\bf x_{k}}},
$$
depends only on those samples that violates the maximum margin criteria, here captured by the fact that $\alpha_k>0$ only for those points laying either inside the margin band or in the wrong side of the boundary, the so-called *support vectors*. For all other points, $\alpha_k=0$. Hence none of them exert any influence on the coefficients determinancy. This property provide SVM a less dependency to outliers. For more details on how to compute $\alpha$ and $\hat{a}_0$, see @Cortes1995, @Hastie2009, and @James2013.
...
$$
f(x)=\hat{a}_0+\hat{{\bf a}}^\intercal\phi({\bf x}),
$$
...
$$
\hat{{\bf a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\bf x_{k}})},
$$
which substituting in $f$ expression above gives us
$$
f(x)=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\bf x_{k}})^\intercal\phi({\bf x})}.
$$
Moreover, the linearity of the model implies that the function $f$ can be expressed as dot products without change its properties:
$$
f(x)=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\bf x_{k}})\cdot{}\phi({\bf x})}.
$$

However, the enlargement of the feature space can be computationaly expensive. SVM makes use of *kernels* to accomplish this idea. Kernel functions are equivalent to dot products of a larger degree feature space vectors. Hence, we can express $f$ as [@Cortes1995]:
$$
f({\bf x})=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}K({\bf x_{k}}, {\bf x})},
$$
...
$$
K({\bf u},{\bf v})=\exp(-\gamma\lVert{{\bf u}-{\bf v}}\rVert^2),
$$
one of the most used nonlinear kernels, maps the input space to an infinite dimensional feature space, which would be roughly obtained by a power series expansion with substantially more calculations. Other popular choices for $K$ in the SVM literature are the polynomial kernel ($K({\bf u},{\bf v})=(\gamma{{\bf u}^{\intercal}{\bf v}}+r)^d$, with parameters $\gamma$, $r$, and $d$), and the linear kernel $K({\bf u},{\bf v})={\bf u}^{\intercal}{\bf v}$. The use of kernels are an efficient computational strategy to produce different boundaries shapes in the input attribute space. Moreover, this stragey generally this achieves better training-class separation.

In `sits`, SVM is the default machine learning model. The following example ilustrate how to classify an individual sample (in this case a serie of 16 one-year NDVI time series from the same location). As a training data we used the NDVI time series from Mato Grosso Brazilian state.  
...
Random forest provides better performances over *bagged trees*, a similar procedure that does not implement stochast feature selection (*i.e.* when $m=p$). Bagged trees suffer from high correlation among trees introduced by an eventual presence of strong predictors that are deterministically chosen as splitting nodes criterion [@James2013]. However, the random forest classification performance can vary according to the tunning of the model parameters. The main methods parameters are the number of attributes randomly sampled as candidates at each split ($m$), the number of decision trees to grow ($b$), the sample size to draw at each bootstrap iteration
