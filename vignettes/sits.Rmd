...
sits_plot(
    sits_merge(
        sits_merge(point_env1.tb, 
                   point_env2.tb),
        sits_linear_interp(point.tb, 24)))
```
...
Support Vector Machine (SVM) is a generalization of the *separating hyperplane classifier* [@Hastie2009]. In its simpler formulation, SVM finds linear boundaries in the input feature space. A boundary is a hyperplane that divides the entire feature space into two parts. Hence, if the feature space has $p$ attributes, the hyperplane has dimension $p-1$ and can be written as
$$
a_0+a_1x_{1}+a_2x_{2}+\ldots+a_px_{p}=0,
$$
where $x_{i}$ is the $i$-th attribute of the $x$ sample vector. 

Let $y$ be its class and assume that all samples falls into two classes $y\in[-1,1]$. In SVM formulation, the hyperplane has the classification property that, in *most of the cases*, the following relation holds
$$
a_0+a_1x_{1}+a_2x_{2}+\ldots+a_px_{p}<0,\quad{}{y=-1},\\
a_0+a_1x_{1}+a_2x_{2}+\ldots+a_px_{p}>0,\quad{}{y=1}.
$$
That is, he hyperplane divides the attribute space in order to separate the samples from distinct classes. However, this must be achieved by maximizing the margin $M$ between the hyperplane and the closest sample vectors. 

The hyperplane coeficients can be determined by an optimization problem: maximize the margin between ... for which a margin between it and the sample vectors is maximized...
and that the perpendicular distance to any given sample $(x, y)$ is given by
$$
d(x, y)=(a_0+a_1x_{1}+a_2x_{2}+\ldots+a_px_{p})y,
$$
which can be achieved by restricting the coeficients to
$$
\sum_{i=1}^{p}{a_{i}^2}=1.
$$
...
