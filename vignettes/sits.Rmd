...
# Web Services for Raster data

One of the distinguishing features of `sits` is that it has been designed to work with big satellite image data sets which reside on the cloud. Many *R* packages that deal with remote sensing images require data to be accessible in a local computer. However, with the coming of age of big Earth observation data, it is not practical to transfer large data sets. Therefore, the package uses the concept of web services. We focus on two kinds of services: those that provide time series data and those that provide access to data cubes. Currently, `sits` works with four services:

* WTSS (Web Series Time Service). WTSS is a light-weight service for retrieval of time series data for selected locations and periods [@Vinhas2016]. The functions include listing the data sets available in the server, describing the contents of a data set, and retrieving a time series based on spatial location and temporal filters.
* SATVEG service.  The SATVEG service provides NDVI and EVI time series vegetation indices from MODIS image for the whole Brazilian territory and has been developed by EMBRAPA (Brazil's Agriculture Research Agency)[@Embrapa2014].
* EOCUBES service. The EOCUBES service provides access to big data collections, which are organised by the `EOCubes` package. 
* RASTER service: the RASTER service is used to create coverages, based on Raster Bricks and associated temporal information. This service should be use for those that want to use `sits` based on files (which can be local or remote).
...
We use these services to build coverages. A coverage is the data type used in `sits` to handle dense raster data. Its definition corresponds to that of a data cube. Following and generalising @Appel2019, we consider that a data cube is a four-dimensional array with dimensions x (longitude or easting), y (latitude or northing), time, and bands. Its spatial dimensions refer to a single spatial reference system (SRS). Cells of a data cube have a constant spatial size (with regard to the cube’s SRS). The temporal dimension is specified by a set of intervals. For every combination of dimensions, a cell has a single value. 

Coverages in `sits` are created using the function `sits_coverage`, which is based on the connection to a data service. The return of this function is a tibble with the metadata about the coverage that lists the contents of the data set, including source, bands, spatial extent and resolution, time range, and temporal resolution. All functions that process coverage are based on lazy evaluation. Since the contents of a coverage can be big, its cells are not loaded into memory. It is the responsibility of each function that processes coverage to do the appropriate data management and processing. Therefore, it can be argued that `sits` coverages are equivalent to the arrays defined by @Appel2019 with lazy evaluation.

```{r}
# create a coverage from WTSS
...
# create a coverage from EOCUBES services
...
coverage_raster <- sits_coverage(service = "RASTER", name = "Sinop",
...
Once a coverage has been created, it can be used for extracting time series and for data analysis and classification. 
