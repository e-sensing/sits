...
Hyperplanes are linear $(p-1)$-dimensional boundaries and define linear partitions in the attribute space. However, sometimes we are faced with nonlinear relationship between attribute vectors and classes. In such cases, linear classification models can suffer to do a good job. An alternative to this limitation is the *enlargement of the attribute space* by a nonlinear mapping to a higher degree feature space. SVM makes use of *kernels* to accomplish this idea. Under some conditions, kernel functions can be expressed as a dot product in a larger degree feature space. In this manner, the new classification model
$$
f(x)=\hat{a}_0+\hat{a}\phi(x),
$$
...
$$
\hat{a}=\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi(x_{k})}.
$$
The linearity of the model implies that the function $f$ can be expressed only as dot products [@Cortes1995]:
$$
\begin{split}
f(x)&=\hat{a}_0+\hat{a}\phi(x)\\
&=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi(x_{k})\cdot{}\phi(x)}\\
&=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}K(x_{k}, x)},
\end{split}
$$
where $K(u,v)$ is a kernel function that, under some conditions, is equivalent to the dot product $\phi(u)\cdot\phi(v)$ (see @Cortes1995). It follows that, we can construct different kernel functions without the need to compute $\phi$, otherwise it would require massive computations. For example, the radial basis function (RBF) kernel
$$
K(u,v)=\exp(-\gamma\lVert{u-v}\rVert^2),
$$
one of the most used nonlinear kernels, maps the input space to an infinite dimensional feature space, which would be roughly obtained by a power series expansion with substantially more calculations. Other popular choices for $K$ in the SVM literature are the polynomial kernel ($K(u,v)=(\gamma{u^{\intercal}v}+r)^d$, with parameters $\gamma$, $r$, and $d$), and the linear kernel $K(u,v)=u^{\intercal}v$. The use of kernels are an efficient computational strategy to produce different boundaries shapes in the input attribute space. Moreover, this stragey generally this achieves better training-class separation.
