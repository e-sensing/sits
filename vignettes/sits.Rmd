...
Earth observation satellites provide a regular and consistent set of information about the land and oceans of the planet. Recently, most space agencies have adopted an open data policy, making unprecedented amounts of satellite data available for research and operational use. This data deluge has brought about a major challenge: *How to design and build technologies that allow the Earth observation community to analyse big data sets?*
...
In the following example, we classify the same data set using the default values of the `deep learning` method, which are: (a) Three layers with 400, 200, 100 neurons each; (b) Using the 'relu' activation function and 'optimizer_adam'; (c) dropout rates of 40%, 30% and 20% for the layers. 
...
plot(sits_get_raster(raster.tb, 1))
```
...
The first parameter is "blocksize". It controls the size of the data block to be read from disk for each file. When acessing raster data, the block size is a multiple of the number of columns in each band. Consider an image consisting of 4 bands of 4000 x 4000 pixels with 400 time steps. A block size of 1000000 will result in using blocks of size 250 lines x 4000 columns per each time instance. Considering there are 4 bands, 200 time steps and that R will store this data as a numeric value (8 bytes), R will need about 12.8 GBytes to store this data in memory for each block. Considering a typical factor of 150% of additional memory used for storing copies of the block, we can estimate that the server will need about 32 GB of continuous memory to be used by R. 
...
Based on current experience, the classification of a MODIS tile (4800 x 4800), with 4 bands and 400 time instances, using a SVM with a training data set of about 10,000 samples, takes about 24 hours using 20 cores and a blocksize of 1e+06, in a server with 2.4GHz Xeon CPU and 96 GB memory.
