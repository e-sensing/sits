...
One of the main challanges in work with cluster anaysis on satellite image time series is to deal with data with high-variability and incomplete information. Since observations of the same land cover class differs from year to year, robusts similarity/dissimilarity functions are need and, in this regard, the use of the shape-based distance function *dynamic time warping* (DTW) has been spreaded in many applications. So is in time series clustering literature. Research on time series data mining shows that methods based on DTW have achieved significant results in many applications [@Esling2012]. DTW works by comparing two time series and finds their optimal alignment, providing a dissimilarity measure as a result [@Mueller2007]. DTW provides a robust distance measure for comparing time series, even if they are irregularly sampled or are out of phase in the time axis [@Keogh2005], hence diminishing the influence of intra-class variability. 
...
The ultrametric relation formed by the successive merges in the hierarchical process is represented by a *dendrogram* tree. Dendrograms are quite useful to decide on the number of clusters has the data [@Hennig2015]. The dendrogram shows the height where each merging happened, which corresponds to the minimum distance between two clusters defined by a *linkage criterion*. The most commom linkage criteria are *single-linkage*, *complete-linkage*, *average-linkage*, and *Ward-linkage*. Complete-linkage prioritises the within-cluster dissimilarities, producing clusters with shorter distance samples. Complete-linkage clustering can be sensitive to outliers, that can increase the resulting intracluster data variance. As an alternative, Ward proposes a criteria to minimise the data variance by means of either *sum-of-squares* or *sum-of-squares-error* [@Ward1963]. Ward's intuition is that clusters of multivariate observations, such as time series, should be approximately elliptical in shape. The linkage criterion used to generate the dendrogram can be informed to `sits_dendro()` and the default option is the *Ward-linkage*. The following code shows how to create and visualise a dendrogram in SITS.
...
dendro <- sits_dendro(cerrado_2classes)
# plot the resulting dendrogram
sits_plot_dendro(cerrado_2classes, 
                     dendro, 
                     cutree_height = 22)
```

In our example above, we can separate our samples by cutting the dendrogram tree at some level to achieve a balancing between two objectives: get clusters as large as possible, and get clusters as homogeneous as possible. To help this process, SITS package provides `sits_dendro_validity()` which computes a series of external validity indexes for a given range of number  [], there is no an automatic way of achieving it [@Liao2005][@Aghabozorgi2015]. This process can be realised visually by looking the dendrogram or assisted by an *external cluster validity indexes* such as Rand Index ... as shows the cut height dashed line in our example.
...
It is possible to rename clusters by calling `sits_cluster_rename()` function. ...remove the outliers samples to avoid further confusion. This can be accomplished with the function `sits_cluster_cleaner()` that removes all samples whose label (cluster) counts less than a given percentual relative to its cluster (label). 
...
The filtering algorithms provided by SITS package are: Savitzky–Golay filter (`sits_sgolay()`), Whitaker filter (`sits_whittaker()`), envelope filter (`sits_envelope()`) and, the "cloud filter" (`sits_cloud_filter()`). In what follows, we explain each technique and provide some examples using the afore described PRODES data set. To best visualize the effects of filtering we select only NDVI band of the time series.
