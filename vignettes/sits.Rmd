...
$$
\hat{{\bf a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}{\bf x}_{k}},
$$
...
$$
\hat{{\bf a}}=\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\bf x}_{k})},
$$
which substituting in $f$ expression above gives us
$$
f(x)=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}\phi({\bf x}_{k})^\intercal\phi({\bf x})}.
$$
Moreover, the linearity of the model implies that the term $\phi({\bf x}_{k})^\intercal\phi({\bf x})$ can be expressed as a dot product $\phi({\bf x}_{k})\cdot{}\phi({\bf x})$ without change its properties.
...
$$
f({\bf x})=\hat{a}_0+\sum_{k=1}^{n}{\alpha_{k}y_{k}K({\bf x}_{k}, {\bf x})},
$$
