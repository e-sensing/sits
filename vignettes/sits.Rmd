...
Approaches such as pre- and post-processing might be used to achieve more accurate maps. Package sits allows . sits allows to exploit a clustering method as pre-processing step using specifically self-organizing maps (SOM) to assess land cover samples of vegetation indexes time series. Independent of classifer, these techniques are suitable for assisting users to select representative land cover change samples from vegetation indexes time series through metrics that indicate sample quality and separability. 
...
To train the network, the neurons are initialized randomly. The vector of weights of neuron $j$,  $w_j=[w_{j1},\ldots,w_{jn} ]$,  must have the same dimension of a input vector of time series samples $x(t)=[x(t)_1,\ldots,x(t)_n ]$. At each training step $t$, a time series sample $x(t)$ is presented to the network to find the neuron whose weight vector has the smaller distance among them. Distance metrics are utilized to compute distance $D_{j}$ between input vector $x(t)$ and each weight vector of neuron, $w_j$ for each neuron j in the output layer. The most part of literature uses Euclidean distance as metric.
