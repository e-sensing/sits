...
After importing the samples time series, it is useful to explore the data and see how is it underlying structured and its inter-class separability. For example, We can note in the figure above the variability of 400 time series samples along time. Those samples were collected from different years and/or locations. The scattering behaviour is intrinsic to remote sensing data. Atmospheric noise, interferences on observations or different equipaments specifications, as well as the very nature of the climate-land dynamics can be sources of such variability [@Atkinson2012]. This can jeopardize the separability of the classes and hence the importance to explore the data. In the following section we present a cluster techinique to help in this task. 

<!--% Do ponto de vista metodológico, é melhor apresentar as técnicas de agrupamento antes das técnicas de filtragem. -->

<!--% principal motivação para o uso do agrupamento é a detecção de anomalias nas amostras. -->

# Clustering in satellite image time series

These samples are usually provided by experts. They need to be tested for separability. Given a set of samples, we need good clustering methods and statistical analysis to find out if these samples can actually serve as a basis for good quality classification.
