...
The Whitakker filter can be a large but sparse optimisation problem, as we can note from $D$ matrix. In `sits` implementation, the default $\lambda{}$ parameter is $1.0$. <!-- @Zhou2016 used different values for $\lambda{}$. For its purpose $\lambda=15$ gives he best result. --> were used in Shoe1 The example bellow depicts an original and a filtered time series.
