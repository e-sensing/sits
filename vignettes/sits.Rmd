...
In random forest algorithm, each growing tree splits its nodes by a *stochastic feature selection*. At each node splitting, it selects $m$ attributes at random from the existing full set of $p$ attributes of the input space. According to a *purity criterion*, the best attribute ${\hat{m} \in \{1,\ldots,p\}}$ is selected among the $m$ candidates (${1 < m \le p}$), which then is used to split that node at some value $S$ of the chosen attribute that minimizes the impurity index. Let ${R_j \subseteq \mathbb{R}^p}$ be the input space region associated with a node $j$. We denote by ${\{\{rm x} \in R_j\}}$ the set of all observations vectors comprehended by the region $R_j$. The splitting step creates a partition on the node region containing two subregions ${R_l_j,R_r_j \subset R_j}$ such that
$$
\begin{split}
R_l_j \cap R_r_j &= \empty\\
R_j &= R_l_j \cup R_r_j\\
R_l_j&=\{{\rm x} | x_{\hat{m}} \le S,\ \forall {\rm x} \in R_j \}\\
R_r_j&=\{{\rm x} | x_{\hat{m}} > S,\ \forall {\rm x} \in R_j \}.
\end{split}
$$
...
$$
E(j)=-\sum_{q \in Q}{P_{qj} \log P_{qj}}.
$$
Finally, the misclassification error impurity index of a node $j$ can be obtained by
$$
M(j)=1-P_{q^*j},
$$
where ${q^*={\rm arg\!max}_{q}{P_{qj}}}$ is the majority class in the node $j$. A consequence of these formulations, cross-entropy and the Gini index are more sensitive to changes in the node classes proportions than the misclassification error [@Hastie2009].
...
The main random forest parameters are the number of attributes sampled as candidates at each split ($m$), the number of decision trees to grow ($b$), the minimum node size ($N_{m\!i\!n}$), and the sample fraction to be drawed at each bootstrap iteration ($\lambda$). Lower values of $m$ tends to grow taller and uncorrelated decision trees, but decreases the accuracy
