...
The `sits_envelope()` can combine both maximization and minimization window sequencially. The function can receive a string sequence with "U" (for maximization) and "L" (for minimization) characters passed to its parameter. A repeated sequence of the same character is equivalent to one operation with a larger window. The sequencial operations on the input time series produces the final result that is returned.

This filter can be useful to remove isolated noises in the input signal. For example, an downward (upward) noise peaks formed by one point in the series can be removed by the "UL" ("LU") operations sequence. For peaks with two points, we can repeat each operation, for example "UULL" (the default sequence), to increase the window length and remove such imperfections of the time series. In the following example we can see an application of `sits_envelope()` function.
...
The maximum operator of envelope filter resembles the maximum value composite [Holben1986] technique, commonly used to enhance NDVI images but here, the process is adapted to a time series context.
...
```{r, fig.align="center", fig.height=2.5, fig.width=4.1}
# Take the NDVI band of the first sample data set
point.tb <- sits_select(prodes_226_064[1,], bands = c("ndvi"))
# apply ARIMA filter
