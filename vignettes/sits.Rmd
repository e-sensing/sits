...
In an investigation of phenological parameter estimation, @Atkinson2012 compare the application of Fourier transform, asymmetric Gaussian, double logistic and the Whittaker filters to preprocess MERIS NDVI time series for estimating phenological parameters in India. They found out that the Whittaker and Fourier transform approaches were preferable to the double logistic and asymmetric Gaussian models.

Using the same filters, @Shao2016 compare the impact of filtering on land use and land cover classification accuracy. The authors used MODIS NDVI time series from the Great Lakes region in North America. As in the previous work, they concluded that only Fourier transform and Whittaker techniques improved interclass separability for crop classes and significantly improved overall classification accuracy. 

In a time series reconstruction investigation, @Zhou2016 compare the Fourier transform, double logistic, asymmetric Gaussian, Whittaker and Savitzky-Golay filters methods for noise removal in MODIS NDVI data from sites spreaded worldwide in different climatological conditions. The authors found that asymmetric Gaussian model outperforms other models over high latitude boreal biomes, while the Savitzky-Golay model gives the best reconstruction performance in tropical evergreen broadleaf forests. In the remaining biomes, Whittaker gives superior results. Other techniques can be found in the literature such as curve fitting [@Bradley2007] and wavelet decomposition [@Sakamoto2005].
...
The Savitzky-Golay filter works by fitting successive sub sets of $l$ (odd number) adjacent data points with a low-degree polynomial whose coefficients can be derived through linear least squares. The central point $i$ of the sub set assumes the value of the interpolated polynomial. An equivalent and much faster solution to this iterated process can be obtained by the expression
$$
{\hat{x}_{i}=\sum _{j=-n}^{n}C_{j}\,x_{i+j}},
$$
where $\hat{x}$ is the the filtered time series, $C_{j}$ are the Savitzky-Golay smoothin coefficients, and $x$ is the original time series. 

The coeficients $C_{j}$ depends uniquely on polynomial degree $d$ and the length $l$ of the sub set of adjacent data points (including the central point). If $d=0$, the coeficients are constants $C_{j}=1/l$ and the Savitzky-Golay filter is equivalent to moving average filter. When the time series is equally spaced, the coefficients have analytical solution (a list of coefficients and an explanation on how to compute them can be found in @Gorry1990). The following example shows the effect of Savitsky-Golay filter on the original time series.
...
The filter has one parameter In `sits` implementation, the default $\lambda{}$ parameter is $1.0$. <!-- @Zhou2016 used different values for $\lambda{}$. For its purpose $\lambda=15$ gives he best result. --> were used in Shoe1 The example bellow depicts an original and a filtered time series.
...
<!-- qual a referencia na literature ::: MVC!!! -->
