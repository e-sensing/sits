...
The main random forest parameters are the number of attributes sampled as candidates at each split ($m$), the number of decision trees to grow ($b$), the minimum node size ($N_{\!m\!i\!n}$), and the sample fraction to be drawed at each bootstrap iteration ($\lambda$). Lower values of $m$ tends to grow taller and uncorrelated decision trees, but decreases the model classification accuracy. 
