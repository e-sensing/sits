...
However, the condition for which this classifier has solution is too restrictive, as it needs a linearly separable data set. This is hardly what one may expect to find in a typical satellite image time series context, where nonlinear relationship between classes and predictors can be find. To overcome this undesired constraint, SVM introduces a softner term in the hyperplane optimization problem. This modification allow some observations to be on the incorrect part of the attribute space violating the maximum margin classifier but incresing the robustness against individual observations influence. The solution for the hyperplane coefficients
$$
\hat{a}_0,\ldots,\hat{a}_p
$$ 
takes into consideration only those attribute points that violates the *maximum margin* criteria, points that lays either in the margin or in the wrong side of the attribute space, the so-called *support vectors*. All other points does not exert any influence on $\hat{a}$ determinancy.
