---
title: "SITS - Satellite Image Time Series Analysis for Data Cubes"
...
- Supports  machine learning methods for time series classification, including SVM, GLM, Lasso, Random Forests, Extreme Gradient Boosting (XGBoost).
- Supports Deep Learning methods for classification: multi-layer perceptrons, 1D Convolution Neural Networks (1D-CNN), tempCNN and ResNet.
...
```
We can then see the cluster frequency using `sits_cluster_frequency`. In the example, we note that cluster $3$, unlike other clusters, includes a mix of two classes. Users can then remove this cluster with `sits_cluster_remove`to reduce the number of mixed-class samples.
```{r}
