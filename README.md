...
Those that want to evaluate the `sits` package before installing it are
invited to run the examples available on
[Kaggle](https://www.kaggle.com/esensing/code). These examples provide
an fast-track introduction to the package. We recommend running them in
the following order:
...
a local machine. The ARD collections accessible with SITS version 0.15.0
are:
...
2.  Brazil Data Cube (BDC): Open data collections of Sentinel-2,
    Landsat-8 and CBERS-4 images in BDC.
3.  Digital Earth Africa (DEAFRICA): Sentinel-2/2A and Landsat-8
    collections available in Digital Earth Africa. These collections are
    openly acessible but not regular;
4.  USGS: Landsat-4/5/7/8. These collections are not open data.

Open data collections do not require payment of access fees to cloud
providers. Except for those in the BDC, These collections are not
regular and require further processing before they can be used in SITS.

### Transforming ARD Image Collections into Data Cubes

SITS works best with regular *data cubes* that meet the following
definition:
...
The user defines a generic data cube by selecting a collection in a
cloud service and then specifying a space-time extent. For example, the
following code will define a data cube of Sentinel-2/2A images using
AWS.
...
by `sits_regularize()` which uses the [gdalcubes
package](https://github.com/appelmar/gdalcubes) package \[4\].
